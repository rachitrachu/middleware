...
from middlewared.plugins.xnas_connect.update import XnasConnectService
from middlewared.plugins.xnas_connect.hostname import TNCHostnameService
from middlewared.plugins.xnas_connect.utils import TNC_IPS_CACHE_KEY


@pytest.fixture
def tnc_service():
    """Create a mock XnasConnectService instance."""
    service = XnasConnectService(MagicMock())
...
class TestTNCHostnameService:
    ...
    async def test_register_update_ips_uses_effective_ips(self):
        ...
        def mock_middleware_call(method, *args, **kwargs):
            if method == 'xnas_connect.config_internal':
                return {'jwt_token': 'test_token'}
            elif method == 'xnas_connect.get_effective_ips':
        ...
        with patch('middlewared.plugins.xnas_connect.hostname.register_update_ips',
    ...
    async def test_register_update_ips_ha_prepends_vips(self):
        ...
        def mock_middleware_call(method, *args, **kwargs):
            if method == 'xnas_connect.config_internal':
                ...
                return True
            elif method == 'xnas_connect.ha_vips':
                return ['10.0.0.100']
            return None
    ...
    async def test_sync_ips_proceeds_with_wildcards(self):
        ...
        async def mock_middleware_call(method, *args, **kwargs):
            ...
            if method == 'failover.is_single_master_node':
                ...
                raise KeyError('miss')
            elif method == 'xnas_connect.hostname.register_update_ips':
