...
class HttpService(PseudoServiceBase):
    ...
    async def _register_new_port(self):
        ...
        if port_changed and (await self.middleware.call("xnas_connect.config"))["status"] == Status.CONFIGURED.name:
    ...
    async def _register_port_with_retry(self, new_port: int):
        ...
        for attempt in range(3):
            try:
                await self.middleware.call('xnas_connect.hostname.register_system_config', new_port)
                return
