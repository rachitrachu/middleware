...
class XnasConnectStateService(Service):

    class Config:
        namespace = 'xnas_connect.state'
        private = True
    ...
    async def check(self, restart_ui=False):
        ...
        if tnc_config['status'] == Status.REGISTRATION_FINALIZATION_WAITING.name:
            ...
            await self.middleware.call('xnas_connect.finalize.status_update', Status.REGISTRATION_FINALIZATION_FAILED)
            ...
            self.middleware.create_task(self.middleware.call('xnas_connect.acme.initiate_cert_generation'))
            ...
            self.middleware.create_task(self.middleware.call('xnas_connect.acme.update_ui'))
