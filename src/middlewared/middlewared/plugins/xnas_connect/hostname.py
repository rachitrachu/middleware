...
from xnas_connect_utils.hostname import hostname_config, register_update_ips, register_system_config
...
class TNCHostnameService(Service):
    ...
    class Config:
        namespace = 'xnas_connect.hostname'
        private = True
    ...
    async def config(self):
        return await hostname_config(await self.middleware.call('xnas_connect.config_internal'))
    ...
    async def register_update_ips(self, ips=None, create_wildcard=False):
        ...
        if ips is None:
            ips = await self.middleware.call('xnas_connect.get_effective_ips')

        if await self.middleware.call('system.is_ha_capable'):
            # For HA based systems, we want to ensure that VIP(s) always get added
            ips = (await self.middleware.call('xnas_connect.ha_vips')) + ips
    ...
    async def sync_ips(self, event_details=None):
        ...
        async with _sync_lock:
            effective_ips = await self.middleware.call('xnas_connect.get_effective_ips')
            ...
            try:
                await self.middleware.call('xnas_connect.hostname.register_update_ips')
            except CallError:
                logger.error('Failed to update IPs with X-NAS Connect', exc_info=True)
            else:
                await self.middleware.call('cache.put', TNC_IPS_CACHE_KEY, effective_ips, 60 * 60)
                await self.middleware.call_hook('xnas_connect.hostname.updated', await self.config())
...
async def update_ips(middleware, event_type, args):
    ...
    _pending_sync = asyncio.get_event_loop().call_later(
        ...
        lambda: middleware.create_task(
            middleware.call('xnas_connect.hostname.handle_update_ips', event_type, args)
        ),
    )
...
async def on_general_config_update(middleware, *args, **kwargs):
    ...
    """
    try:
        await middleware.call('xnas_connect.hostname.sync_ips')
    except Exception:
