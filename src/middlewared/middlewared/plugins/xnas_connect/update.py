...
from xnas_connect_utils.config import get_account_id_and_system_id
from xnas_connect_utils.status import Status
from xnas_connect_utils.urls import get_account_service_url

import middlewared.sqlalchemy as sa
from middlewared.api import api_method, Event
from middlewared.api.current import (
    XnasConnectEntry, XnasConnectUpdateArgs, XnasConnectUpdateResult,
...
from .private_models import XnasConnectUpdateEnvironmentArgs, XnasConnectUpdateEnvironmentResult
from .utils import CLAIM_TOKEN_CACHE_KEY, get_unset_payload, TNC_IPS_CACHE_KEY

logger = logging.getLogger('xnas_connect')


class XnasConnectTier(enum.IntEnum):
    FOUNDATION = 1
    PLUS = 2
    BUSINESS = 3


class XnasConnectModel(sa.Model):
    __tablename__ = 'xnas_connect'
...
class XnasConnectService(ConfigService, TNCAPIMixin):

    class Config:
        datastore = 'xnas_connect'
        datastore_extend = 'xnas_connect.config_extend'
        cli_private = True
        namespace = 'xnas_connect'
        entry = XnasConnectEntry
        role_prefix = 'TRUENAS_CONNECT'
        events = [
            Event(
                name='xnas_connect.config',
                description='Sent on X-NAS Connect configuration changes',
                roles=['TRUENAS_CONNECT_READ'],
                models={
                    'CHANGED': XnasConnectConfigChangedEvent,
                },
            )
        ]
    ...
    async def config_extend(self, config):
        ...
        try:
            config['tier'] = XnasConnectTier(config['registration_details']['tier']).name
    ...
    @api_method(XnasConnectUpdateArgs, XnasConnectUpdateResult, audit='X-NAS Connect: Updating configuration')
    async def do_update(self, data):
        ...
        self.middleware.send_event('xnas_connect.config', 'CHANGED', fields=new_config)

        return new_config

    @api_method(XnasConnectUpdateEnvironmentArgs, XnasConnectUpdateEnvironmentResult, private=True)
    async def update_environment(self, data):
        ...
        return await self.middleware.call('xnas_connect.config')
    ...
    async def unset_registration_details(self, revoke_cert_and_account=True):
        ...
        await self.middleware.call('xnas_connect.acme.revoke_cert')
    ...
    @api_method(
        XnasConnectIpsWithHostnamesArgs, XnasConnectIpsWithHostnamesResult, roles=['TRUENAS_CONNECT_READ']
    )
    async def ips_with_hostnames(self):
        """
        Returns current mapping of ips configured with truenas connect against their hostnames.
        """
        hostname_config = await self.middleware.call('xnas_connect.hostname.config')
        return {
    ...
    async def set_status(self, new_status, db_payload=None):
        ...
        self.middleware.send_event('xnas_connect.config', 'CHANGED', fields=(await self.config()))
