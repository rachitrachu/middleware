...
from xnas_connect_utils.urls import get_heartbeat_url

from middlewared.alert.source.xnas_connect import (
...
class TNCHeartbeatService(Service, TNCAPIMixin):
    ...
    class Config:
        namespace = 'xnas_connect.heartbeat'
        private = True

    async def call(self, url, mode, payload=None, **kwargs):
        config = await self.middleware.call('xnas_connect.config_internal')
                ...
                match resp['status_code']:
                    ...
                    case 401:
                        ...
                        with contextlib.suppress(Exception):
                            ...
                            await self.middleware.call('xnas_connect.unset_registration_details', False)

                        await self.middleware.call('datastore.update', 'xnas_connect', tnc_config['id'], {
                            'enabled': False,
                        } | get_unset_payload())
                        self.middleware.send_event(
                            'xnas_connect.config', 'CHANGED', fields=await self.middleware.call('xnas_connect.config')
                        )
                        await self.call2(self.s.alert.oneshot_create, TNCDisabledAutoUnconfiguredAlert())
                        await self.middleware.call('xnas_connect.delete_cert', tnc_config['certificate'])
                        return
                    case 500:
