...
from xnas_connect_utils.urls import get_registration_finalization_uri
...
class TNCRegistrationFinalizeService(Service, TNCAPIMixin):
    ...
    class Config:
        namespace = 'xnas_connect.finalize'
        private = True

    async def status_update(self, status, log_message=None):
        await self.middleware.call('xnas_connect.set_status', status.name)
        if log_message:
            ...
            if status['error'] is None:
                ...
                if 'token' not in status['response']:
                    ...
                    await self.middleware.call(
                        'datastore.update', 'xnas_connect', config['id'], {
                            'jwt_token': token,
