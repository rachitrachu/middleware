...
class TNCPostInstallService(Service):
    ...
    class Config:
        private = True
        namespace = 'xnas_connect.post_install'
    ...
    async def process(self, post_install_config):
        ...
        cert_id = await self.middleware.call(
            'datastore.insert',
            'system.certificate', {
                'name': f'xnas_connect_{str(uuid.uuid4())[-5:]}',
        ...
        asyncio.get_event_loop().call_later(
            5 * 60,
            lambda: self.middleware.create_task(self.middleware.call('xnas_connect.hostname.sync_ips')),
        )
