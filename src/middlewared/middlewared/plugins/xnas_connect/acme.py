...
from xnas_acme_utils.ari import fetch_renewal_info
from xnas_connect_utils.acme import acme_config, create_cert
from xnas_connect_utils.exceptions import CallError as TNCCallError
from xnas_connect_utils.status import Status
from xnas_crypto_utils.read import get_cert_id
...
logger = logging.getLogger('xnas_connect')
...
class TNCACMEService(Service):
    ...
    class Config:
        private = True
        namespace = 'xnas_connect.acme'

    async def config(self):
        return await acme_config(await self.middleware.call('xnas_connect.config_internal'))

    async def update_ui(self, start_heartbeat=True):
        logger.debug('Updating UI with TNC cert')
        config = await self.middleware.call('xnas_connect.config')
        if config['certificate'] is None:
            # Just some sanity testing
            logger.error('TNC cert configuration failed')
            await self.middleware.call('xnas_connect.set_status', Status.CERT_CONFIGURATION_FAILURE.name)
        else:
            logger.debug('TNC cert configured successfully')
            await self.middleware.call('xnas_connect.set_status', Status.CONFIGURED.name)
            if start_heartbeat:
                logger.debug('Initiating TNC heartbeat')
                self.middleware.create_task(self.middleware.call('xnas_connect.heartbeat.start'))
    ...
    async def initiate_cert_generation(self):
        ...
        try:
            ...
            await self.middleware.call('xnas_connect.set_status', Status.CERT_GENERATION_FAILED.name)
        else:
            ...
            await self.middleware.call(
                'xnas_connect.set_status', Status.CERT_GENERATION_SUCCESS.name, {'certificate': cert_id}
            )
    ...
    async def renew_cert(self, bypass_renewal_check=False):
        ...
        if not bypass_renewal_check:
            renewal_needed, cert_renewal_id = await self.middleware.call('xnas_connect.acme.check_renewal_needed')
        ...
        await self.middleware.call('xnas_connect.set_status', Status.CERT_RENEWAL_IN_PROGRESS.name)
        try:
            config = await self.middleware.call('xnas_connect.config')
            renewal_job = await self.middleware.call(
                'xnas_connect.acme.create_cert', config['certificate'], cert_renewal_id
            )
            await renewal_job.wait(raise_error=True)
        except Exception:
            logger.error('Failed to renew certificate for TNC', exc_info=True)
            await self.middleware.call('xnas_connect.set_status', Status.CERT_RENEWAL_FAILURE.name)
        else:
            ...
            await self.middleware.call('etc.generate', 'ssl')
            await self.middleware.call('xnas_connect.set_status', Status.CERT_RENEWAL_SUCCESS.name)
    ...
    def check_renewal_needed(self):
        ...
        config = self.middleware.call_sync('xnas_connect.config')
        if config['certificate'] is None:
        ...
        acme_config = self.middleware.call_sync('xnas_connect.acme.config')
    ...
    async def initiate_cert_generation_impl(self):
        cert_job = await self.middleware.call('xnas_connect.acme.create_cert')
        await cert_job.wait()
        if cert_job.error:
    ...
    async def create_cert(self, job, cert_id=None, cert_renewal_id=None):
        ...
        resp = await self.middleware.call('xnas_connect.hostname.register_update_ips', None, True)
        try:
            return await create_cert(
                await self.middleware.call('xnas_connect.config_internal'), resp['response'] or {},
    ...
    async def revoke_cert(self):
        tnc_config = await self.middleware.call('xnas_connect.config_internal')
        ...
        acme_config = await self.middleware.call('xnas_connect.acme.config')
        if acme_config['error']:
            self.logger.error(
...
async def check_status(middleware):
    ...
    await middleware.call('xnas_connect.state.check')
