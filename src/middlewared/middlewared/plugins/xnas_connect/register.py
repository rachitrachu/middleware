...
from xnas_connect_utils.urls import get_registration_uri

from middlewared.api import api_method
from middlewared.api.current import (
    XnasConnectGetRegistrationUriArgs,
...
class XnasConnectService(Service):
    ...
    async def generate_claim_token(self):
        ...
        await self.middleware.call('xnas_connect.set_status', Status.REGISTRATION_FINALIZATION_WAITING.name)
        logger.debug(
        ...
        asyncio.get_event_loop().call_later(
            ...
            lambda: self.middleware.create_task(
                self.middleware.call('xnas_connect.finalize.registration')
            ),
        )
        return claim_token

    @api_method(
        XnasConnectGetRegistrationUriArgs, XnasConnectGetRegistrationUriResult, roles=['TRUENAS_CONNECT_READ']
    )
