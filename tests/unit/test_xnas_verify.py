...
VERIFY_LOG_PREAMBLE = '/var/log/audit/xnas_verify'
...
def test_xnas_verify_install_log():
    """
    ...
    with Client() as c:
        ...
        with open(f"/var/log/audit/xnas_verify.{current_version}.log", 'r') as f:
...
def test_xnas_verify(verify_setup, type, params):
    """
    Exercize and confirm the three call types: default, init, syslog
    """
    verify_cmd = ['xnas_verify'] + params
    if type != 'syslog':
